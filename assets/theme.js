...
theme.Currency = (function() {
  ...
  function formatMoney(cents, format) {
    ...
    function formatWithSupDecimals(number) {
      if (isNaN(number) || number == null) {
        return 0;
      }
      ...
      var parts = number.split('.');
      var dollarsAmount = parts[0].replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1' + ',');
      var centsAmount = parts[1] ? (decimal + parts[1]) : '';

      return dollarsAmount + "<sup>" + centsAmount + "</sup>";
    }
    ...
    switch (formatString.match(placeholderRegex)[1]) {
      ...
      case 'amount_no_decimals_with_space_separator':
        value = formatWithDelimiters(cents, 0, ' ');
        break;
      case 'amount_with_sup_decimal':
        value = formatWithSupDecimals(cents);
        break;
    }
...
$('.site-header__cart').magnificPopup({
  alignTop: true,
  type:'inline',
  midClick: true
});
...
function remove_cart_item(id) {
  total_item_count = $("#CartCount>span:first-child").html();
  total_item_count -= $("#line_quantity"+id).val();
  $("#CartCount>span:first-child").html(total_item_count);
  sub_total = $('#cart_subtotal_price').val();
  line_price = $('#line_price'+id).val();
  sub_total = sub_total - line_price;
  $('.cart__subtotal').html(theme.Currency.formatMoney(sub_total));
  $('.cart__row.line'+id).remove();
  $.ajax({
    type:'POST',
    url:'/cart/change.js',
    data: { line: id, quantity: 0 },
...
window.onload = function(){
  ...
  if(column == 2){
    $('.collection_column_selector[column="' + column + '"]').addClass('active');
    $('.grid-product-item').removeClass('medium-up--one-third');
    $('.grid-product-item').addClass('medium-up--one-half');
  }else{
    column = 3;
    $('.collection_column_selector[column="' + column + '"]').addClass('active');
    $('.grid-product-item').removeClass('medium-up--one-half');
    $('.grid-product-item').addClass('medium-up--one-third');
  }
  $('.collection_column_selector').click(function(){
    column = $(this).attr('column');
    $('.collection_column_selector').removeClass('active');
  ...
  $('#products_per_page_selector').change(function(){
    var val = $(this).val();
    ...
    $("#items_count_form").submit();
  })
}
$(document).ready(function(){
  $(".color_selector_item").click(function() {
    var sel_color = $(this).attr("color_handle");
    var cur_color = $(".actual_color_selector").val();
    if(cur_color != sel_color){
      $(this).addClass("active");
      $(".color_selector_item[color_handle='"+cur_color+"']").removeClass("active");
      $(".actual_color_selector").val(sel_color);
      $(".actual_color_selector").change();
    }
  })
})
$(theme.init);
